...
func main() {
	...
	pb.RegisterWebsiteServiceServer(s, &server{db})
...
func loadConfig() error {
	...
	viper.SetDefault("APP_NAME", "web/cms-website")
...
func createDB(sqlDb *sql.DB) error {
	...
	s = append(s, `USE cms;`)
	s = append(s, `CREATE TABLE IF NOT EXISTS website_content
		(
		...
		engine=innodb;`)
	s = append(s, `CREATE TABLE IF NOT EXISTS website_category
		(
			id          INT(11) NOT NULL auto_increment,
		...
		engine=innodb;`)
	s = append(s, `CREATE TABLE IF NOT EXISTS website
		(
			id		INT(11) NOT NULL auto_increment,
			name		VARCHAR(256) NOT NULL,
