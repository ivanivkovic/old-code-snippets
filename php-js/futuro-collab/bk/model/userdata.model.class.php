...
class modelUserData implements libBaseModel
{
	...
	*/
	...
	public static function getCompleteData( $id )
	{
		...
		{
			$data['joined-string'] = libTemplate::formatTimeString( $data['datejoined'] );
			$data['joined-tag'] = libTemplate::formatTimeTag( $data['datejoined'] );
			$data['lastlogin-string'] = libTemplate::formatTimeString( $data['lastlogin'] );
			$data['lastlogin-tag'] = libTemplate::formatTimeTag( $data['lastlogin'] );
	...
	*/
	...
	{
		$data = Core::$db->fetchSQL( 'SELECT userid, name, lastname FROM ?user WHERE (level="0" OR level ="1") AND status="1" ORDER BY name DESC' );
