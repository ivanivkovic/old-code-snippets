jQuery.poskok.prompt = 
{
	Ask : function ( options )
	{
		...
		{
			$(PopUpFooter).append($('<a href="#close" class="btn pull-right">Odustani<span></span></a>').click(function()
			{
				Settings.onClose(); $(this).parents('.modal').modal('hide'); return false;
			}));
			
			$(PopUpFooter).append($('<a href="#ok" class="btn btn-primary pull-right margin-right-10">Potvrdi<span></span></a>').click(function()
			{
		...
		{
			$(PopUpFooter).append($('<a href="#ok" class="btn pull-right">Zatvori<span></span></a>').click(function()
			{
				Settings.onClose(); $(this).parents('.modal').modal('hide'); return false;
			}));
		}
