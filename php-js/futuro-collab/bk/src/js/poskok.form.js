function removeTag( obj ) // Napravljen za tagove. Briše iz input type hidden-a vrijednosti.
{
...
jQuery(function($)
{
	$('.form .files li').live('click', function(e)
	{
		$(this).find('input').click();
	});
	
	$('.form .files input[type="checkbox"]').live('click', function(e)
	{
		e.stopPropagation();
	});
	
	/// INPUT TYPE FILE!
	$('.input-file button').live('click', function(e)
	{
		e.preventDefault();
	...
	$('.input-file input').live('change', function()
	{
		var $domobj = $(this);
		...
		$(this).parent().find('span').text(count + ' datoteka odabrano');
	});
});