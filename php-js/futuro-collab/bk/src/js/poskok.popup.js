jQuery.poskok.popup = 
{
	Ask : function ( options )
	{
		...
		{
			$(PopUpFooter).append($('<button class="btn pull-right">Odustani</button>').click(function()
			{
				Settings.onClose(); $(this).parents('.modal').modal('hide'); return false;
			}));
			
			$(PopUpFooter).append($('<button class="btn btn-primary pull-right margin-right-10">Potvrdi</button>').click(function()
			{
		...
		{
			$(PopUpFooter).append($('<button class="btn pull-right" data-dismiss="modal" aria-hidden="true">Zatvori</button>').click(function()
			{
				Settings.onClose(); $(this).parents('.modal').modal('hide'); return false;
			}));
		}
