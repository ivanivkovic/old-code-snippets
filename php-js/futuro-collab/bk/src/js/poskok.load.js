...
jQuery(function($)
{
	...
	setTimeout(function()
	{
		$('#success-list').fadeOut(siteSettings.widgetFade);
	},
	3000);
	...
	$('input, textarea').live('keypress keydown', function(e)
	{
		e.stopPropagation();
	...
	$('#global-search').live('keyup', function(e)
	{
		if( this.value !== '' )
		{
			$('.search-results .lead').html('Rezultati pretrage za <strong>"' + this.value + '"</strong>:');
	...
	// Form stuffz
	$('.href-checkbox').live('click', function()
	{
		if( $(this).prev().is('input:checkbox') )
		{
			$(this).prev().click();
		}
	});
	
	// Divs with hover-display buttons.
	$('.button-exit').live('mouseenter',
		function()
		{
			var Box = '<a class="close" href="#" onclick="return false;">Zatvori</a>';
			$(this).prepend(Box);
		}
	);
	
	$('.button-exit').live('mouseleave', 
		function()
		{
	...
	$('.button-hide').live('mouseenter',
		function()
		{
			var Box = '<a class="hide" href="#" onclick="return false;">Sakrij</a>';
			$(this).prepend(Box);
		}
	);
	
	$('.button-hide').live('mouseleave', 
		function()
		{
	...
	{
		$(this).parent().fadeOut(siteSettings.widgetFade, function() {  $(this).remove();  });
	});
	...
	$('select').live('DOMMouseScroll mousewheel', function( e )
	{
