...
class modelClient implements libBaseModel
{
	...
	public static function getCompleteData($clientid)
	{
		...
		$clientData['active-tasks'] = count( self::getActiveTasks( $clientData['clientid'] ) );
		
		return $clientData;
	}
	
	public static function getActiveTasks( $clientId )
	{
		$sql = 'SELECT taskid FROM ?task JOIN ?project ON ?project.clientid = "' . $clientId . '" AND ?project.projectid = ?task.projectid WHERE ?task.status="1"';
	...
	public static function getPage( $param )
	{
		...
		foreach( $clientList as &$client )
		{
			...
			$client['active-tasks'] = count( modelClient::getActiveTasks( $client['clientid'] ) );
		}
