...
class modelTask implements libBaseModel
{
	...
	*/
	...
	public static function getCompleteData( $taskId )
	{
		...
		$data['subTasks'] = self::getSubTasks( $data['taskid'] );
	...
	public static function getNumPages()
	{
		...
		$query .= ( $where === true ? ' AND ' : ' WHERE ' ) . 'parent="0"';
	...
	public static function getPage()
	{
		...
		$query .= ' ORDER BY ?task.status DESC,
						CASE ?task.status WHEN 1 THEN deadline END ASC,
						CASE ?task.status WHEN 0 THEN timefinished END DESC
		...
		foreach( $data as $key => $item )
		{
			...
			$items[$key]['project'] = modelProject::getData( $item['projectid'], array( 'title', 'projectid') );
	...
	// Ako je korisnik već ubačen kao assigned, nemoj ništa napraviti.
	...
	public static function getAssignedWorkers( $taskId )
	{
		$query = 'SELECT
				...
				JOIN ?user ON ?task_assignment.userid = ?user.userid
				WHERE ?task_assignment.taskid="' . $taskId . '"';
	...
	{
		libTemplate::set( 'autocompleteList', modelUserData::getUsersForAutocomplete( array( 0, 1, 2 ) ) );
	...
	public static function getSubTaskData( $taskId )
	{
		return Core::$db->fetchOne( '*', 'task', array('taskid' => $taskId) );
	}
	
	public static function getSubTasks( $taskId )
	{
		$data = Core::$db->fetch( 'taskid', 'task', array('parent' => $taskId), '', array('publishtime' => 'DESC') );
		
		$tasks = array();
		foreach($data as $item)
		{
			$tasks[] = self::getSubTaskData( $item['taskid'] );
		}
	...
	public static function getUserHotTasks( $userId )
	{
		$sql = 'SELECT
					?task.taskid, description
					FROM ?task
					JOIN ?task_assignment ON ?task_assignment.userid = "' . $userId . '" AND ?task_assignment.taskid = ?task.taskid 
					WHERE ?task.status = 1
					ORDER BY deadline ASC LIMIT 15
		';
		
		$data = Core::$db->fetchSQL($sql);
	...
	public static function statusForm( $data )
	{
		$taskId = $data[1];
		...
		libTemplate::loadTemplateFile( Conf::DIR_WIDGETS . 'ajaxforms/task-status-update.php' );
	}

	public static function updateStatus( $taskId, $data )
	{
		if( ! self::exists($taskId) || ! ( self::isOwner($taskId, Core::$user->id || Core::$user->level === 0 ) ))
		{
			return false;
		}
		...
		if( $data['status'] === '0' )
		{
			$data['timefinished'] = libTemplate::convertTimeString($data['timefinished']);
		}
		else
		{
			$data['timefinished'] = 0;
	...
	public static function newSubTaskForm( $data )
	{
		$taskId = $data[1];
		
		libTemplate::loadTemplateFile( Conf::DIR_WIDGETS . 'ajaxforms/subtask-new.php', array('taskId' => $taskId) );
	}
	...
	public static function updateForm( $data )
	{
		...
		libTemplate::set(
			array(
				'taskData' => $taskData,
				'autocompleteList' => modelUserData::getUsersForAutocomplete(array(0, 1, 2)),
	...
	public static function postNewSubTask()
	{
		$subTask['taskid'] = $_POST['taskid'];
		$subTask['description'] = $_POST['description'];
		
		if( is_numeric( $subTask['taskid'] ) && self::exists( $subTask['taskid'] ) )
		{
			$insert = self::createNewSubTask( $subTask );
			
			if( is_numeric($insert) )
			{
				ob_start();
				
				$subTask = self::getSubTaskData( $insert );
				
				include('app/includes/subtask.php');
				
				$html = ob_get_clean();
				
				return array('error' => 0, 'subtaskid' => $insert, 'html' => $html);
			}
		}
	}

	public static function createNewSubTask( $data )
	{
		$parentData = Core::$db->fetchOne( array('projectid', 'recieverid'), 'task', array('taskid' => $data['taskid']));
		
		$insertId = Core::$db->insert(
			array(
				'parent' => $data['taskid'],
				'description' => $data['description'],
				'projectid' => $parentData['projectid'],
				'creatorid' => Core::$user->id,
				'publishtime' => libDateTime::Time(),
				'status' => '1',
				'recieverid' => $parentData['recieverid']
			),
			'task'
		);
